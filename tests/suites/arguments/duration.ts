...
export const duration = (client: Client) => describe('Duration', () => {
  ...
  it('should default to days if the unit is invalid', async () => {
    const args = await Argument.cast('duration', client.commands.resolver, message, '8g');
    expect(args).toEqual(TIME.MS_PER_DAY * 8);
  });
